...
**/
   * FUNCION que Devuelve la diferencia de a??os meses y dias de Las fechas pasadas como par?ímetros, en un string de 6 posiciones con formato ddmmaa
...
return {"Lunes","Martes","Miercoles","Jueves","Viernes","S?íbado","Domingo"}[nDia]
 
**/
   * FUNCION que Averigua el nombre del d?¡a de la fecha o n??mero de d?¡a enviado.
 
   * @author  Sanrom's Software de M?®xico 14/08/97
   * @param   xFecha  Par?ímetro que indica la fecha o N??mero del d?¡a a convertir. por defecto Date()
   * N O T A S  : El par?ímetro podr?í ser una fecha v?ílida o un n??mero comprendido entre 1 y 7.
...
**/
   * FUNCION que Suma D?¡as a una fecha tomando solo d?¡as H?íbiles util paar plazos administrativos
 
   * @author  Sanrom's Software de M?®xico 14/08/97
   * @param   <fFecha>  - Par?ímetro que indica la fecha desde la cu?íl se va a Partir.
   ...
   * @param   <fFecha2> - Indica una fecha hasta la cual se quiere llegar para regresar los d?¡as h?íbiles entre esas fechas
   * N O T A S  : Si no se manda la fecha o los d?¡as regresa la fecha del sistema o el siguiente dia habil.
   
   * @return  fFechaFIn   Una fecha con los d?¡as sumado, tomando solo d?¡as h?íbiles.
*/
 
FUNCTION DiaHabil( fFecha, nDias, nDato, fFecha2)
   LOCAL cAno
   LOCAL nHabil                    // Contador de D?¡as H?íbiles, hasta llegar a nDias enviados.
   LOCAL fFechaFin                 // Fecha con los d?¡as ya sumados para ser h?íbil
   LOCAL nNoHabil := 0             // D?¡as que no son h?íbiles para incrementar a nDias
 
   DEFAULT fFecha TO DATE()      // Por default tomar  la fecha del sistema.
   DEFAULT nDias TO 0           // Por default no incrementa d?¡as
   DEFAULT nDato TO 1           // Por default regresa la fecha h?íbil
   ...
   FOR nHabil := 0 TO nDias
       ...
       ** Si la fecha es domingo, sabado o un d?¡a festivo, incrementa los dias a sumar a la fecha
       IF DOW(fFechaFin) == 1 .OR. DOW(fFechaFin) == 7 .OR. fFechaFin == CTOD("01/01/" + cAno) .OR.;
          ...
          ++nNoHabil   // Incrementa los d?¡as que no son h?íbiles para sumarlos a los dias solicitados
          --nHabil     // Decrementa el contador de los d?¡as, para dar otra vuellta y ver si cae en d?¡a H?íbil
       ENDIF
   ...
   DO CASE
      CASE nDato == 1                         // Regresa la Fecha en D?¡as H?íbiles
           fFechaFin := fFecha + (nNoHabil + nDias)
 
      CASE nDato == 2
           fFechaFin := nNoHabil + nDias      // Regresa los d?¡as reales que se necesitan suma para ser h?íbil
 
      CASE nDato == 3                         // Regresa los d?¡as que nos son h?íbiles
           fFechaFin := nNoHabil
 
      CASE nDato == 4                         // Regresa los d?¡as que son h?íbiles ente 2 fechas
           fFechaFin := nHabil
   ENDCASE
...
**/
   ...
   * @author  Bingen Ugaldebere
   * @param   dFechaIni  Par?ímetro que indica la fecha de inicio
   * @param   dFechaFin  Par?ímetro que indica la fecha de final
   * @param   aFestivos  [Opcional] Array con las festividades a tener en cuenta entre ambas fechas
   * @param   lIncluirSabados [Opcional] Si los s?íbados se tienen en cuenta como laborales, por defecto .F.
...
**/
   ...
   * @param   <xFecha>  - Par?ímetro que indica la fecha o N??mero del mes a convertir.
   * @param   <<nDias>  - N??mero de d?¡as que se le van a sumar, si no se manda, se suma 0
   * @param   <nDato>   - Dato que se desea 1 = Fecha, 2 = Dias a Sumar
   * @param   <fFecha2> - Indica una fecha hasta la cual se quiere llegar para regresar los d?¡as h?íbiles entre esas fechas
   * N O T A S  : El par?ímetro podr?í ser una fecha v?ílida o un n??mero comprendido entre 1 y 12.
...
**/
   ...
   * @param   <fFechaFin> -> Segunda Fecha a tratar. Si se omite este par?ímetro, se tomar?í la fecha actual con DATE().
   * @param   <nRegresa>  -> Indica que tipo de dato se est?í solicitando: 1 = A??os  2 = A??os y Meses  3 = A??os, Meses y D?¡as
   * @param   <cSeparador> -> Caracter a utilizar como separador entre los A??os, Meses y D?¡as. Por default ser?í: "."
...
**/
   ...
   * @param   <xFecha>  = Par?ímetro que indica la fecha o Cadena del mes a convertir.
 
   * N O T A S  : El par?ímetro podr?í ser una fecha v?ílida o una cadena con el nombre del mes.
